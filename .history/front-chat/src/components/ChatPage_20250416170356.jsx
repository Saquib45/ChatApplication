    ...
    useEffect(()=>{
        if(chatBoxRef.current){
            chatBoxRef.current.scroll({
                top:chatBoxRef.current.scrollHeigh
            })
        }
    },[messages])
     ...
     return(
        <div className="">
            ...
            <header className=" dark:border-gray-700 fixed w-full dark:bg-gray-900 py-5 flex justify-around items-center">
                ...
                <div>
                    <button onClick={handleLogout} className="dark:bg-red-500 hover:dark:bg-red-700 px-3 py-2 rounded-full text-gray-300">Leave Room</button>
                </div>
            </header>

            {/* chat container */}
            <main ref={chatBoxRef} className="py-20 px-10 border w-2/3 dark:bg-slate-600 text-gray-300  mx-auto h-screen overflow-auto">
                {
